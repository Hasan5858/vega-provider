 ...
 * - Send eval to deobfuscation API
 * - Parse m3u8 from deobfuscated code
 */
function streamhgExtractor(url, axios, signal) {
    return __awaiter(this, void 0, void 0, function () {
        var idMatch, id, embedUrl, res, html, directM3u8, evalCode, evalPattern, evalMatch, apiResp, data, deob, m3u8Match, m3u8Url, cleanUrl, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    res = _a.sent();
                    html = res.data || "";
                    // Try direct m3u8 in embed HTML first
                    directM3u8 = html.match(/https?:\/\/[^"'\s]+\.m3u8[^"'\s]*/i);
                    if (directM3u8) {
                        return [2 /*return*/, { link: directM3u8[0].replace(/&amp;/g, "&"), headers: { "User-Agent": USER_AGENT, Referer: embedUrl }, type: "m3u8" }];
                    }
                    evalCode = null;
                    // Grab the eval body only (without <script> tags)
                    // Flexible PACKER pattern (allow spaces and both quotes)
                    ...
                    if (!!evalCode) return [3 /*break*/, 2];
                    console.error("StreamHG: no eval block found in embed page");
                    return [2 /*return*/, null];
                case 2:
                    // First try built-in PACKER unpack
                    try {
                        // Try group-capture unpack first (more robust for nested parentheses)
                        ...
                        if (deob) {
                            ...
                            if (m3u8Url) {
                                ...
                                return [2 /*return*/, { link: cleanUrl, headers: { "User-Agent": USER_AGENT, Referer: embedUrl }, type: "m3u8" }];
                            }
                        }
                    }
                    catch (_) { }
                ...
                case 4:
                    ...
                    if (!data.success || !data.result) {
                        console.error("StreamHG: deobfuscation API failed", data.error || data);
                        return [2 /*return*/, null];
                    }
                    deob = String(data.result);
                    // Find any m3u8 URL in the deobfuscated code
                    m3u8Match = deob.match(/https?:\/\/[^"'\s]+\.m3u8[^"'\s]*/i);
                    m3u8Url = m3u8Match ? m3u8Match[0] : null;
                    if (m3u8Url) {
                        cleanUrl = m3u8Url.replace(/&amp;/g, "&");
                    ...
                    console.error("StreamHG: no m3u8 found in deobfuscated code");
                    return [2 /*return*/, null];
                case 5:
                    error_1 = _a.sent();
                    console.error("StreamHG: deobfuscation API error", error_1);
                    return [2 /*return*/, null];
...
function unpackPacker(code) {
    try {
        ...
        // Safely materialize the argument list
...
// Extract by capturing the 4 main arguments directly: payload, base, count, words
