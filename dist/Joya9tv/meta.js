...
const getMeta = function (_a) {
    return __awaiter(this, arguments, void 0, function* ({ link, providerContext, }) {
        ...
        try {
            ...
            if (/S\d+|Season \d+|TV Series\/Shows/i.test(infoContainer.find("h1").text() + $(".sgeneros").text())) {
                result.type = "series";
            }
            else {
                result.type = "movie";
            ...
            downloadTable.find("tr").each((index, element) => {
                ...
                if (quality && directLink) {
                    ...
                    const seasonMatch = (_a = rawTitle.match(/S(\d+)/)) === null || _a === void 0 ? void 0 : _a[1];
                    let fullTitle = `${result.title}`;
                    if (seasonMatch)
                        fullTitle += ` Season ${seasonMatch}`;
                    fullTitle += ` - ${quality}`;
                    if (size)
                        fullTitle += ` (${size})`; // ADDED: Append size to the link title
                    links.push({
                        title: fullTitle,
