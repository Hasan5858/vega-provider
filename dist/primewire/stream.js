...
class PrimewireBlowfish {
    ...
    de(value) {
        ...
        for (let i = 0; i < blockCount; i++) {
            const block = value.substr(i * 8, 8);
            if (block.length < 8)
    ...
    xor(l, r) { return this.fixNeg(l ^ r); }
    addMod32(l, r) { return this.fixNeg((l + r) | 0); }
    fixNeg(v) { return v >>> 0; }
    split64by32(v) {
...
function decodeLinkKeys(encoded) {
    ...
    try {
        const bf = new PrimewireBlowfish(key);
        const decrypted = bf.decryptBase64(payload);
        return (_b = (_a = decrypted.match(/.{1,5}/g)) === null || _a === void 0 ? void 0 : _a.map(s => s.trim()).filter(Boolean)) !== null && _b !== void 0 ? _b : [];
    }
    catch (error) {
...
function handlePrimeSrcEmbed(url, axios, cheerioModule) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const embedRes = yield axios.get(url);
            const $ = cheerioModule.load(embedRes.data);
            const streams = [];
            $('video source, source[src*=".mp4"], source[src*=".m3u8"]').each((index, el) => {
...
const extractMixdrop = (mixdropUrl, axios) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const embedUrl = mixdropUrl.replace("/f/", "/e/");
        const { data } = yield axios.get(embedUrl, {
            headers: {
        ...
        const decode = function (payload, a, c, k, e, d) {
            e = function (index) {
...
const extractDoodStream = (doodUrl, axios) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const response = yield axios.get(passUrl, {
            headers: {
                "User-Agent": USER_AGENT,
                Referer: embedUrl,
                Accept: "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8",
...
const extractStreamTape = (streamTapeUrl, axios) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const { data } = yield axios.get(streamTapeUrl, {
            headers: {
                "User-Agent": USER_AGENT,
        ...
        const html = data;
        const directMatch = (_a = html.match(/id="robotlink"[^>]*>([^<]+)</)) !== null && _a !== void 0 ? _a : html.match(/document\.getElementById\('robotlink'\)\.innerHTML\s*=\s*'([^']+)'/);
        ...
        const rawLink = directMatch[1]
            ...
            .replace(/&amp;/g, "&")
            .trim();
        const normalized = rawLink.startsWith("http") || rawLink.startsWith("https")
            ? rawLink
            : rawLink.startsWith("//")
                ? `https:${rawLink}`
                : rawLink.startsWith("/")
                    ...
                    : rawLink;
        const finalUrl = normalized.includes("&stream=") || normalized.includes("stream=")
            ? normalized
            : `${normalized}&stream=1`;
        return {
            link: finalUrl,
...
const resolveGoEntries = (url, $, axios) => __awaiter(void 0, void 0, void 0, function* () {
    const baseUrl = new URL(url).origin;
    const linkKeys = decodeLinkKeys($("#user-data").attr("v") || "");
    const entries = [];
    $("a.go-link").each((_, element) => {
    ...
    for (const entry of entries) {
        ...
        let goData;
        try {
            const response = yield axios.get(goUrl, {
                headers: {
                    "User-Agent": USER_AGENT,
                    Referer: url,
        ...
        const directLink = typeof goData === "string" ? goData : (goData === null || goData === void 0 ? void 0 : goData.link) || (goData === null || goData === void 0 ? void 0 : goData.url) || null;
        if (!directLink) {
            continue;
        }
        const hostLabel = ((goData === null || goData === void 0 ? void 0 : goData.host) || entry.host || "Primewire").trim();
        const host = hostLabel.toLowerCase();
        let extracted = null;
        if (directLink.includes("mixdrop") || host.includes("mixdrop")) {
            extracted = yield extractMixdrop(directLink, axios);
            if (extracted) {
                extracted.type = extracted.type || "mp4";
            }
        }
        else if (directLink.includes("dood") || host.includes("dood")) {
            extracted = yield extractDoodStream(directLink, axios);
            if (extracted) {
                extracted.type = extracted.type || "mp4";
            }
        }
        else if (directLink.includes("streamtape") ||
            directLink.includes("streamta") ||
            host.includes("streamtape")) {
            extracted = yield extractStreamTape(directLink, axios);
            if (extracted) {
                extracted.type = extracted.type || "mp4";
            }
        }
    ...
    return __awaiter(this, arguments, void 0, function* ({ link: url, type, providerContext, }) {
        ...
        try {
            if (url.includes("primesrc.me")) {
                return yield handlePrimeSrcEmbed(url, axios, cheerio);
            }
            const pageResponse = yield axios.get(url, {
                headers: {
                    "User-Agent": USER_AGENT,
                    Accept: "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8",
                    "Accept-Language": "en-US,en;q=0.9",
            ...
            if (mixdropCandidates.length) {
                ...
                for (const candidate of mixdropCandidates) {
                    const extracted = yield extractMixdrop(candidate.link, axios);
                    if (extracted) {
                        streams.push({
