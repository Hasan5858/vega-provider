...
class PrimewireBlowfish {
  ...
  private de(value: string): string {
    ...
    for (let i = 0; i < blockCount; i++) {
      ...
      if (block.length < 8) throw new Error("Invalid block length");
  ...
  private xor(l: number, r: number): number { return this.fixNeg(l ^ r); }
  private addMod32(l: number, r: number): number { return this.fixNeg((l + r) | 0); }
  private fixNeg(v: number): number { return v >>> 0; }
  
  private split64by32(v: string): [number, number] {
...
function decodeLinkKeys(encoded: string | null | undefined): string[] {
  if (!encoded) return [];
  ...
  try {
    ...
    return decrypted.match(/.{1,5}/g)?.map(s => s.trim()).filter(Boolean) ?? [];
  } catch (error) {
